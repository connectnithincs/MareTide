...
import { History, Eye, ShieldAlert, Clock, RefreshCw, GitCommit } from "lucide-react";
import { OperationTimeline } from "./OperationTimeline";

export const HistoryLogs: React.FC = () => {
  const [activeTab, setActiveTab] = useState<"operations" | "vision" | "timeline">("timeline");
  ...
  return (
    <div className="flex-1 overflow-y-auto p-6 space-y-6 bg-brand-dark">
      ...
      <div className="flex items-center justify-between border-b border-brand-border pb-4">
        <div>
          <h2 className="text-xl font-black text-brand-text tracking-wide uppercase">Historical Logs & Audit Traceability</h2>
          <p className="text-xs text-brand-muted font-semibold mt-1">Review chronological operations history, end-to-end container timeline, and safety audit logs.</p>
        </div>
        {activeTab !== "timeline" && (
          <button 
            onClick={fetchData}
      ...
      <div className="flex gap-1 border-b border-brand-border">
        ...
        >
          ...
          <span>Operation Timeline (10-Stage)</span>
        </button>
        <button
